document.addEventListener("DOMContentLoaded", function () {
  ...
  function handleButtonClick(card) {
    ...
    const url = `volunteerRequestsDetails.html?id=${card.id}&organization=${encodeURIComponent(card.organization)}&address=${encodeURIComponent(card.address)}&subject=${encodeURIComponent(card.subject)}&googleMap=${encodeURIComponent(card.googleMap)}&area=${encodeURIComponent(card.area)}&governorate=${encodeURIComponent(card.governorate)}&category=${encodeURIComponent(card.category)}`;
