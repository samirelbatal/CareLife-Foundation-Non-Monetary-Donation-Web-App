...
searchInput.addEventListener("input", () => {
  ...
  function filterCards(searchTerm) {
    ...
    function handleButtonClick(card) {
      ...
      const url = `volunteerRequestsDetails.html?id=${card.id}&address=${encodeURIComponent(card.address)}&telephone=${encodeURIComponent(card.telephone)}&email=${encodeURIComponent(card.email)}&area=${encodeURIComponent(card.area)}&governorate=${encodeURIComponent(card.governorate)}&organization=${encodeURIComponent(card.organization)}`;
