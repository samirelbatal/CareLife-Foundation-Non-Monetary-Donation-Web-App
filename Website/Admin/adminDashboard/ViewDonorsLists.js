function searchUser() {
  var input, filter, table, tr, tdId, tdName, tdEmail, i, tdType;
  ...
  for (i = 0; i < tr.length; i++) {
    ...
    tdType = tdEmail = tr[i].getElementsByTagName("td")[3];
    if (tdId || tdName || tdEmail || tdType) {
      var idMatch = tdId && tdId.textContent.toUpperCase().indexOf(filter) > -1;
      var nameMatch =
        tdName && tdName.textContent.toUpperCase().indexOf(filter) > -1;
      var emailMatch =
        tdEmail && tdEmail.textContent.toUpperCase().indexOf(filter) > -1;
      var typeMatch =
        tdType && tdType.textContent.toUpperCase().indexOf(filter) > -1;
...
function addRow(id, name, email, type) {
  ...
  cell4.innerHTML =
    '<i class="fas fa-user-cog edit-icon" title="Account Management"></i>&nbsp;&nbsp;<i class="fas fa-trash-alt delete-icon" title="Delete"></i>';
...
document
  .getElementById("closeDeleteConfirmationModal")
...
function InsertAllRows() {
  userData.forEach(function (user) {
    addRow(user.id, user.name, user.email, user.type);
  });
}

var userData = [
  { id: 1, name: "John Doe", email: "john@example.com", type: "Donor" },
  { id: 2, name: "Jane Smith", email: "jane@example.com", type: "Doctor" },
  {
    id: 4,
  ...
  { id: 6, name: "Ahmed Hany", email: "michael@example.com", type: "Teacher" },
  {
    id: 7,
    name: "yousef khalid",
    email: "michael@example.com",
  ...
  { id: 8, name: "john yousef", email: "michael@example.com", type: "Donor" },
];
